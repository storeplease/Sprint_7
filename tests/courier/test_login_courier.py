...
class TestLoginCourier:
    ...
    def test_login_courier_success(self, registered_courier):
        courier_methods, params = registered_courier
        response_data, status_code = courier_methods.login_courier({
        "login": params["login"],
        "password": params["password"]
    })
        assert status_code == 200 and "id" in response_data, f"Expected status code 200, got {status_code} or Response does not contain 'id'"
